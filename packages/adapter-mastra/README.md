...
```ts
import { createMastraAdapter } from "@ignitionai/adapter-mastra";
...
```ts
import { mastraAdapter } from "@ignitionai/adapter-mastra";
...
The adapter uses `@ignitionai/adapter-callable` internally, so thrown agent errors flow through the standard experiment failure path.
